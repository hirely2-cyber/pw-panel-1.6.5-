...
@section('title', 'XML — '.$char->name)
@section('breadcrumb', 'Characters / '.$char->name.' / XML Editor')

@section('content')

{{-- highlight.js (view mode) --}}
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/styles/atom-one-dark.min.css">
<script src="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/highlight.min.js"></script>

{{-- CodeMirror 5 (edit mode) --}}
...
<div class="pw-card mb-2">
    <div class="pw-card-body !py-2 flex flex-wrap items-center gap-3">
        ...
        <span class="text-[13px] font-bold">{{ $char->name }}</span>
        <span class="text-[11px] text-[var(--color-text-muted)]">#{{ $char->id }}</span>
        <span class="text-[11px] text-[var(--color-text-muted)]">·</span>
        <span class="text-[11px] text-[var(--color-text-muted)]">Owner: <strong class="text-[var(--color-text)]">{{ $char->username ?? '(deleted)' }}</strong></span>
        <span class="text-[10px] text-[var(--color-text-muted)]">v<strong class="text-[var(--color-text)] font-mono">{{ $version }}</strong></span>
        <div class="ml-auto">
            <a href="{{ route('panel.characters.show', $char->id) }}" class="pw-btn pw-btn-sm">← Back</a>
        </div>
    </div>
</div>
...
<div class="xml-editor-wrap">
    ...
    <div id="xml-edit-wrap">
        <form id="xml-save-form" method="POST" action="{{ route('panel.characters.xml.save', $char->id) }}">
            @csrf
