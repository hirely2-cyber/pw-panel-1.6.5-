...
<div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
    ...
    <div class="pw-card" x-data="pwServicesPanel()" x-init="init()">
        <div class="pw-card-title flex items-center justify-between">
            <span class="inline-flex items-center gap-2">
                ...
                SERVICES
                {{-- Live progress text during action --}}
            ...
            <span class="text-xs font-semibold {{ $anyOnline ? 'text-emerald-600' : 'text-rose-500' }}"
                  x-show="!busy">
                {{ $anyOnline ? 'ONLINE' : 'OFFLINE' }}
            </span>
        </div>

        {{-- Thin progress bar strip under title --}}
        ...
        <div class="pw-card-body">
            ...
            <div class="grid grid-cols-4 gap-2 mb-2">
                ...
                @endphp
                @foreach ($cells as [$lbl,$val,$cls])
                    <div class="rounded border border-[var(--color-border-2)] bg-[var(--color-surface-2,#f7f9fb)] px-3 py-2 text-center">
                        <div class="text-[11px] text-[var(--color-text-soft)]">{{ $lbl }}</div>
                        <div class="font-semibold {{ $cls }}">{{ number_format($val) }} MB</div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="flex items-center justify-between text-[11px] mb-1">
                ...
                <span class="text-[var(--color-text-soft)]">{{ $memory['apps_pct'] }}%</span>
            </div>
            <div class="h-1.5 bg-slate-100 rounded overflow-hidden mb-2">
                <div class="h-full bg-sky-500" style="width: {{ $memory['apps_pct'] }}%"></div>
            </div>

            {{-- Buff/Cache bar --}}
            <div class="flex items-center justify-between text-[11px] mb-1">
                <span class="text-amber-500 font-semibold">Buff/Cache</span>
                <span class="text-[var(--color-text-soft)]">{{ $memory['buff_cache_pct'] }}%</span>
            </div>
            <div class="h-1.5 bg-slate-100 rounded overflow-hidden mb-3">
                <div class="h-full bg-amber-400" style="width: {{ $memory['buff_cache_pct'] }}%"></div>
            </div>
            ...
            <div class="flex items-center justify-between text-[11px] mb-1">
                ...
                <span class="text-[var(--color-text-soft)]">
                    @if ($memory['swap_total'] > 0)
                        {{ $memory['swap_pct'] }}% · {{ number_format($memory['swap_used']) }} / {{ number_format($memory['swap_total']) }} MB
                    @else
                        Not configured
                    @endif
                </span>
            ...
            <div class="h-1.5 bg-slate-100 rounded overflow-hidden mb-4">
                <div class="h-full bg-rose-400" style="width: {{ $memory['swap_pct'] }}%"></div>
            </div>

            {{-- Services rows --}}
            <div class="divide-y divide-[var(--color-border-2)] border border-[var(--color-border-2)] rounded">
            @foreach (config('pw.services') as $key => $svc)
                @php
                ...
                <div class="flex items-center justify-between px-3 py-2 text-sm transition-colors duration-300"
                     :class="{
                         'bg-amber-50': svcState['{{ $key }}'] && svcState['{{ $key }}'].phase === 'running',
                         'bg-emerald-50/60': svcState['{{ $key }}'] && svcState['{{ $key }}'].phase === 'ok' && svcState['{{ $key }}'].label === 'Online',
                         'bg-rose-50/60': svcState['{{ $key }}'] && (svcState['{{ $key }}'].phase === 'fail' || (svcState['{{ $key }}'].phase === 'ok' && svcState['{{ $key }}'].label === 'Offline')),
                     }">
                    <div class="flex items-center gap-2 min-w-0">
                        <span class="font-semibold">{{ $label }}</span>
                        <code class="text-[11px] text-[var(--color-text-soft)]">./{{ $program }}{{ $inst > 1 ? " ×{$inst}" : '' }}</code>
                    </div>

                    <div class="flex items-center gap-3 shrink-0">
                        {{-- RAM usage per service (hidden while action in progress) --}}
                        <span class="text-[11px] text-[var(--color-text-soft)] tabular-nums"
                              x-show="!(svcState && svcState['{{ $key }}'])">
                            @if (($st['rss_mb'] ?? 0) > 0)
                                <span class="text-sky-600 font-semibold">{{ number_format($st['rss_mb']) }}</span>
                            @else
                        ...
                        {{-- LIVE badge: replaces Offline/Online while service is being started --}}
                        <template x-if="svcState && svcState['{{ $key }}']">
                            <span class="pw-badge inline-flex items-center gap-1 transition-all duration-200"
                                  :class="{
                                      'pw-badge-warn': svcState['{{ $key }}'].phase === 'running',
                                      'pw-badge-ok':   svcState['{{ $key }}'].phase === 'ok' && svcState['{{ $key }}'].label === 'Online',
                                      'pw-badge-off':  svcState['{{ $key }}'].phase === 'fail' || (svcState['{{ $key }}'].phase === 'ok' && svcState['{{ $key }}'].label === 'Offline'),
                                  }">
                                {{-- running: spinner --}}
                                <template x-if="svcState['{{ $key }}'].phase === 'running'">
                                    <span class="inline-block w-2.5 h-2.5 border-2 border-white/40 border-t-white rounded-full animate-spin"></span>
                                </template>
                                {{-- ok Online: check --}}
                                <template x-if="svcState['{{ $key }}'].phase === 'ok' && svcState['{{ $key }}'].label === 'Online'">
                                ...
                                {{-- ok Offline: dash --}}
                                <template x-if="svcState['{{ $key }}'].phase === 'ok' && svcState['{{ $key }}'].label === 'Offline'">
                                ...
                                {{-- fail: X --}}
                                <template x-if="svcState['{{ $key }}'].phase === 'fail'">
                                ...
                                <span x-text="svcState['{{ $key }}'].label"></span>
                            </span>
                        </template>

                        {{-- STATIC badge (when no live action for this row) --}}
                        <template x-if="!(svcState && svcState['{{ $key }}'])">
                            @if ($st['online'])
            ...
            <div class="mt-3 text-[11px] text-[var(--color-text-soft)] bg-[var(--color-surface-2,#f7f9fb)] border border-[var(--color-border-2)] rounded px-2 py-1.5">
                ↓ Backup stored at:
                <code class="text-[var(--color-accent)]">{{ rtrim($server->server_path, '/') }}/pw_backup_YYYYMMDD.tar.gz</code>
            </div>
        </div>
    </div>
    ...
    <div x-data="pwMapsPanel()" class="pw-card">
        ...
        {{-- Countdown banner (full-width, prominent) --}}
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <div class="border border-[var(--color-border-2)] rounded overflow-hidden flex flex-col" style="min-height:460px;">
                    ...
                    <ul class="flex-1 overflow-y-auto divide-y divide-[var(--color-border-2)] text-sm" data-list="online" style="max-height:520px;">
                        @forelse ($onlineMaps as $m)
                            <li class="px-2 py-1 flex items-center gap-2 bg-emerald-50/40"
                                x-show="isVisible(@js($m['id']), @js($m['name']))">
                                <input type="checkbox" class="pw-checkbox"
                                       value="{{ $m['id'] }}" x-model="onlineSel">
                                <code class="text-[var(--color-accent)] font-semibold text-xs">{{ $m['id'] }}</code>
                                <span class="text-xs truncate flex-1">{{ $m['name'] }}</span>
                                @if (!empty($m['cpu']))
                ...
                <div class="border border-[var(--color-border-2)] rounded overflow-hidden flex flex-col" style="min-height:460px;">
                    <div class="px-2.5 py-1.5 bg-[var(--color-surface-2,#f7f9fb)] border-b border-[var(--color-border-2)] flex items-center justify-between">
                        <span class="text-[11px] font-semibold text-[var(--color-text-soft)] flex items-center gap-1.5">
                            ...
                            AVAILABLE MAPS ( {{ count($availableMaps) }} )
                        </span>
                        <label class="text-[11px] inline-flex items-center gap-1">
                    ...
                    <ul class="flex-1 overflow-y-auto divide-y divide-[var(--color-border-2)] text-sm" data-list="avail" style="max-height:520px;">
                        @forelse ($availableMaps as $m)
                            <li class="px-2 py-1 flex items-center gap-2"
                                x-show="isVisible(@js($m['id']), @js($m['name']))">
                                <input type="checkbox" class="pw-checkbox"
                                       value="{{ $m['id'] }}" x-model="availSel">
                                <code class="text-[var(--color-primary)] font-semibold text-xs">{{ $m['id'] }}</code>
                                <span class="text-xs truncate flex-1">{{ $m['name'] }}</span>
                            </li>
                        @empty
                            <li class="px-3 py-6 text-center text-xs text-[var(--color-text-soft)]">
...
function pwServicesPanel() {
    return {
        ...
        async runAction(action) {
            ...
            this._svcLabels = {!! json_encode($pwSvcLabels) !!};
            const stopOrder  = {!! json_encode($pwStopOrder)  !!};
            const startOrder = {!! json_encode($pwStartOrder) !!};
            const instMap    = {!! json_encode($pwInstMap)    !!};
            if (action === 'stop') {
            ...
            let res;
            try {
                res = await fetch('{{ route("panel.server.control.stream") }}', {
                    method: 'POST',
                    headers: {
        ...
        instCount(key) {
            const map = {!! json_encode($pwInstMap) !!};
            return map[key] || 1;
        },
...
function pwPostServer(payload) {
    ...
    form.method = 'POST';
    form.action = @json(route('panel.server.control'));
    const add = (n, v) => {
...
function pwBroadcast(message, channel = 9) {
    ...
    return fetch(@json(route('panel.server.broadcast')), {
        method: 'POST',
        headers: {
